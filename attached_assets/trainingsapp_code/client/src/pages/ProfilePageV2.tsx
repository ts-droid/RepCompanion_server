...
import { Loader2, Save, User, Target, Clock, Activity, Sparkles, AlertTriangle } from "lucide-react";
...
export default function ProfilePageV2() {
  ...
  const { data: profile, isLoading: profileLoading, refetch } = trpc.workout.getProfile.useQuery(
    undefined,
    { enabled: !!user }
  );
  ...
  const [initialGoals, setInitialGoals] = useState({ goalVolume: 50, goalStrength: 30, goalCardio: 20 });
  const [initialSettings, setInitialSettings] = useState({ sessionsPerWeek: 3, sessionDuration: 45 });
  ...
  const [hasAiProgram, setHasAiProgram] = useState(false);
  ...
  useEffect(() => {
    if (profile) {
      ...
      if (profile.sessionsPerWeek) {
        setSessionsPerWeek(profile.sessionsPerWeek);
        setInitialSettings(prev => ({ ...prev, sessionsPerWeek: profile.sessionsPerWeek! }));
      }
      if (profile.sessionDuration) {
        setSessionDuration(profile.sessionDuration);
        setInitialSettings(prev => ({ ...prev, sessionDuration: profile.sessionDuration! }));
      }
      if (profile.goalVolume !== undefined && profile.goalVolume !== null) {
        setGoalVolume(profile.goalVolume);
        setInitialGoals(prev => ({ ...prev, goalVolume: profile.goalVolume! }));
      }
      if (profile.goalStrength !== undefined && profile.goalStrength !== null) {
        setGoalStrength(profile.goalStrength);
        setInitialGoals(prev => ({ ...prev, goalStrength: profile.goalStrength! }));
      }
      if (profile.goalCardio !== undefined && profile.goalCardio !== null) {
        setGoalCardio(profile.goalCardio);
        setInitialGoals(prev => ({ ...prev, goalCardio: profile.goalCardio! }));
      }
      if (profile.restTime) setRestTime(profile.restTime);
      if (profile.hasAiProgram) setHasAiProgram(profile.hasAiProgram === 1);
    }
  }, [profile]);

  // Check if training goals or settings have changed
  useEffect(() => {
    if (!hasAiProgram) return; // Only check if user already has a program

    const goalsChanged =
      goalVolume !== initialGoals.goalVolume ||
      goalStrength !== initialGoals.goalStrength ||
      goalCardio !== initialGoals.goalCardio;

    const settingsChanged =
      sessionsPerWeek !== initialSettings.sessionsPerWeek ||
      sessionDuration !== initialSettings.sessionDuration;

    if (goalsChanged || settingsChanged) {
      setPendingChanges(true);
    } else {
      setPendingChanges(false);
    }
  }, [goalVolume, goalStrength, goalCardio, sessionsPerWeek, sessionDuration, hasAiProgram, initialGoals, initialSettings]);
  ...
  const handleGenerateProgram = (autoGenerate = false) => {
    ...
    generateProgramMutation.mutate(
      ...
      },
      {
        onSuccess: (data) => {
          ...
          // Update hasAiProgram flag
          updateProfileMutation.mutate(
            { hasAiProgram: 1 },
            {
              onSuccess: () => {
                setHasAiProgram(true);
                setInitialGoals({ goalVolume, goalStrength, goalCardio });
                setInitialSettings({ sessionsPerWeek, sessionDuration });
                setPendingChanges(false);
                refetch();
              },
            }
  ...
  const handleSave = () => {
    // Check if there are pending changes that require regeneration
    if (pendingChanges && hasAiProgram) {
  ...
  const handleConfirmRegenerate = () => {
    ...
    updateProfileMutation.mutate(
      ...
      },
      {
        onSuccess: () => {
          toast.success("Profil sparad!");
          // Then regenerate program
          handleGenerateProgram(true);
        },
        onError: (error) => {
          toast.error("Kunde inte spara profil: " + error.message);
        },
    ...
    <div className="min-h-screen bg-background">
      <div className="container max-w-4xl py-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={handleSave} disabled={updateProfileMutation.isPending || generateProgramMutation.isPending}>
            {updateProfileMutation.isPending || generateProgramMutation.isPending ? (
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-yellow-800 dark:text-yellow-200 mt-1">
                    Du har ändrat dina träningsmål eller inställningar. När du sparar kommer AI att generera ett nytt träningsprogram automatiskt.
                  </p>
                </div>
              </div>
            </CardContent>
        ...
        {/* AI Program Generation - Only show if no program exists yet */}
        {!hasAiProgram && (
        ...
        <div className="flex justify-end">
          <Button onClick={handleSave} size="lg" disabled={updateProfileMutation.isPending || generateProgramMutation.isPending}>
            {updateProfileMutation.isPending || generateProgramMutation.isPending ? (
              <Loader2 className="mr-2 h-4 w-4 animate-spin" />
            ) : (
              <Save className="mr-2 h-4 w-4" />
      ...
      <Dialog open={showLoadingAnimation} onOpenChange={() => {}}>
        <DialogContent className="max-w-md" showCloseButton={false}>
          <DialogHeader>
            <DialogTitle className="sr-only">Genererar träningsprogram</DialogTitle>
          </DialogHeader>
          <RunningStickman />
        </DialogContent>
      </Dialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              ...
              Regenerera träningsprogram?
            </AlertDialogTitle>
            <AlertDialogDescription>
              Du har ändrat dina träningsmål eller inställningar. Om du vill applicera dessa ändringar kommer AI att generera ett nytt träningsprogram automatiskt.
              <br /><br />
              Vill du fortsätta och låta AI skapa ett nytt program baserat på dina uppdaterade inställningar?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Avbryt</AlertDialogCancel>
            <AlertDialogAction onClick={handleConfirmRegenerate}>
              Ja, generera nytt program
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
